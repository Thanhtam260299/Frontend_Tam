const ELEMENT_FORM = document.getElementById('js-task')

const ELEMENT_BTN = document.getElementById('js-btn')
const ELEMENT_INPUT = document.getElementById('input_value')
const ELEMENT_LEVEL = document.getElementById('input_level')
const ELEMENT_SUBMIT = document.getElementById('input_submit')
const ELEMENT_DELETE = document.getElementById('btn-delete')

var toggleForm
var id = 0

